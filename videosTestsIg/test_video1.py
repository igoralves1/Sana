...
def get_video_config():
    ...
    config = {
        # Model selection - USING LongSANA for better compatibility
        "model_name": "Efficient-Large-Model/LongSANA_1.6B_1024px_diffusers",
        
        # Video duration and quality
        "duration_seconds": 5,       # Length of video in seconds (start with 5 for testing)
        "fps": 16,                   # Frames per second (16, 24, 30, or 60)
        "height": 704,               # Video height in pixels
        "width": 1280,               # Video width in pixels
        
        # Generation quality (higher = better but slower)
        "guidance_scale": 7.0,       # How closely to follow prompt (5-10)
...
def main():
    ...
    print(f"{Colors.BOLD}{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"{Colors.BOLD}{Colors.CYAN}🎬 LongSANA VIDEO CINEMATIC GENERATOR{Colors.END}")
    print(f"{Colors.BOLD}{Colors.CYAN}{'='*60}{Colors.END}")
    ...
    try:
        print(f"\n{Colors.BOLD}📦 Loading LongSANA model...{Colors.END}")
        print(f"   First download takes ~5-8GB - monitoring active!\n")
        ...
        video_filename = f"{script_name}_longsana_{config['duration_seconds']}s_{config['fps']}fps.mp4"
        
        video_np = (video_frames * 255).byte().cpu().numpy()
        imageio.mimsave(video_filename, video_np, fps=config["fps"], codec='libx264')
        ...
        print(f"{Colors.BOLD}{Colors.GREEN}✨ SUCCESS! Your LongSANA cinematic is ready! ✨{Colors.END}")
        print(f"{Colors.BOLD}{Colors.GREEN}{'='*60}{Colors.END}\n")
        ...
        print(f"   🎨 Resolution: {config['width']}x{config['height']}")
        print(f"   🖼️  Total Frames: {config['num_frames']}")
